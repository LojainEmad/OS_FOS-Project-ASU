...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//[3.3] reallocate in a different place (relocate)
	cprintf("	3.3: reallocate in a different place (relocate)\n\n");
	is_correct = 1;

	{
	    blockIndex = 2 * allocCntPerSize;
	    old_size = get_block_size(startVAs[blockIndex]);
	    new_size = allocSizes[3] + allocSizes[4];
	    expected_size = new_size + sizeOfMetaData();
	    expected_va = startVAs[blockIndex];
	    va = realloc_block_FF(startVAs[blockIndex], new_size);
	    //Check return address ,it should be a different place.
	    if (va == NULL || va == expected_va)
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #11.1: WRONG REALLOC -it didn't relocate the block.Expected a different address, Actual %x\n", va);
	    }
	    //Check reallocated block size &status.
	    ...
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #11.3: WRONG REALLOC! block status(is_free)is not equal to 0 after realloc.\n");
	    }
	    // Check content of the reallocated block.
		if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex)
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #11.4: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
	    }
	    // Check the old block, which should be marked as free.
	    if ( is_free_block(expected_va) != 1 )
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #: WRONG REALLOC! the old block is not marked as free .\n");
	    }
	}
	...
	is_correct = 1;
	{
	    ...
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #14.6: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n", startVAs[blockIndex] ,va);
	    }
	    //check reallocated block size & status
	    block_size = get_block_size(startVAs[blockIndex]) ;
	    if (block_size != new_size + sizeOfMetaData())
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #14.7: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
	    }
	    block_status = is_free_block(startVAs[blockIndex]) ;
	    if (block_status != 0)
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #14.8: WRONG REALLOC! block status (is_free) not equal 0 after realloc.\n");
	    }
	    //check if the next block has been coalesced
	    struct BlockMetaData *nextBlkMetaData = (struct BlockMetaData *)(va + new_size);
	    if (nextBlkMetaData->is_free != 1 || nextBlkMetaData->size != old_size - new_size - sizeOfMetaData())
	    {
	        is_correct = 0;
	        cprintf("test_rea`lloc_block_FF #14.9: WRONG REALLOC! next block has not been coalesced correctly.\n");
	    }
	    //check content of reallocated block
	    if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex || *(endVAs[blockIndex]) != blockIndex)
	    {
	        is_correct = 0;
	        cprintf("test_realloc_block_FF #15: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
	    }
	}
