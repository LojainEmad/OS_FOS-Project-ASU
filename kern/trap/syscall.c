...
void* sys_sbrk(int increment)
{
	...
	uint32 NEW_BREAK ;
	uint32 Original_Break = curenv->USER_BREAK;

	if (increment == 0) {
	    	return (void *)curenv->USER_BREAK;
	    }
	if (increment > 0) {
    	increment=ROUNDUP(increment,PAGE_SIZE);
   	    NEW_BREAK = curenv->USER_BREAK + increment;
   	 if (NEW_BREAK < curenv->USER_HARD_LIMIT)
   	 {
     	sys_allocate_user_mem( curenv->USER_BREAK , increment);
     	curenv->USER_BREAK = NEW_BREAK ;
   	 }
     return (void*) Original_Break;
	}
	if(increment <0)
	{
        int va =increment*-1;
        va=ROUNDUP(va,PAGE_SIZE);
        NEW_BREAK = curenv->USER_BREAK-va;
        pt_set_page_permissions(curenv->env_page_directory,NEW_BREAK,0,PERM_MARKED_P);
        return (void *)curenv->USER_BREAK;
	}
	return (void*)-1;
}
bool is_user_range_valid(uint32 addr) {
    // Check if the address is not null and is within the user address space
    return (addr != 0) && (addr <= USER_LIMIT);
}
uint32 sys_get_limit(void)
{
	//return curenv->limit;
	return curenv->USER_HARD_LIMIT;
}

uint32 sys_get_Break(void)
{
	//return curenv->limit;
	return curenv->USER_BREAK;
}

uint32 sys_get_start(void)
{
return curenv->USER_START ;
}

uint32 sys_get_permission(uint32 virtual_address)
{
	uint32 *ptr_page_table ;
	get_page_table(curenv->env_page_directory,virtual_address,&ptr_page_table);
    uint32 x = pt_get_page_permissions(curenv->env_page_directory, virtual_address);

    return x;

}

/*uint32 sys_ptr_pageDirect (void)
{
	//return curenv->limit;
	return (uint32)curenv->env_page_directory;
}*/
void sys_env_set_nice(int sys_nice_val) {
	struct Env* e = curenv;
    env_set_nice(e, sys_nice_val);
}
...
if ((uint32*)a1 == NULL|| a1 >= USER_LIMIT||(a1 >= USER_LIMIT - PAGE_SIZE)){
	sched_kill_env(curenv->env_id);
	cprintf("killl here 1 \n");
	return -E_INVAL;
		}
...
if ((uint32*)a1 == NULL||a1 >= USER_LIMIT||(a1 >= USER_LIMIT - PAGE_SIZE) ){
    ...
    case SYS_get_limit:
        ...
        break;
    case SYS_get_permission:
    	return sys_get_permission((uint32) a1);
    	break;
    case SYS_get_start:
    	    return sys_get_start();
            break;
    case SYS_get_Break:
    	return sys_get_Break();
    	            break;
    case SYS_cputs:
